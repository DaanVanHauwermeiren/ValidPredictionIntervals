...
__all__ = [
    ...
    "Taxonomy",
    "MultisplitTaxonomy",
    "MultiquantileTaxonomy",
    "taxonomyFactory",
    "conditionalCP",
    "Conformalizer",
    "PointConformalizer",
    "EnsembleConformalizer",
    "RandomEnsembleConformalizer",
]
        ...
        if t in np.unique(taxonomy):
            ...
            if len(score_t) == 0:
                warnings.warn(
                    "Class "
                    + str(t)
                    + " contains 0 instances. It might be better to use different splits. Defaulting to threshold 0."
                )
            else:
...
class Conformalizer:
    ...
    def train_conditional(self, x, y, suppress_verbosity=False):
        if self.container.cp_mode():
            self.conditional_thresholds.update(
                conditionalCP(
                    lambda x, y: self.__score(self.train_model, x, y),
                    self.container.taxonomy_func(),
                    x,
                    y,
